 ...
 *
 * 그누보드5 옆에 g5_meeting_api 폴더로 업로드한 후,
 * 브라우저에서 https://YOUR-DOMAIN/g5_meeting_api/setup_board.php?token=YOUR_TOKEN 1회 호출.
 *
 * 작업:
...
$provided = $_GET['token'] ?? '';
if (!meeting_API_TOKEN || !hash_equals(meeting_API_TOKEN, $provided)) {
...
if (!$existing) {
    ...
    sql_query("CREATE TEMPORARY TABLE _tmp_meeting LIKE {$g5['board_table']}");
    sql_query("INSERT INTO _tmp_meeting SELECT * FROM {$g5['board_table']} WHERE bo_table = '$template_bo'");
    sql_query("UPDATE _tmp_meeting SET bo_table = '$bo_table_safe', bo_subject = '회의록', bo_count_write = 0, bo_count_comment = 0");
    sql_query("INSERT INTO {$g5['board_table']} SELECT * FROM _tmp_meeting");
    sql_query("DROP TEMPORARY TABLE _tmp_meeting");
...
api_ok([
    'message' => '게시판 준비 완료',
    'next_steps' => [
        '1. config.local.php 의 meeting_API_TOKEN 을 강력한 값으로 변경',
