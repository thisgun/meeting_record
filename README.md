...
C:\xampp\htdocs\g5_meeting_api    ──Junction──►  C:\dev2\g5_meeting_api
```
...
│       → http://127.0.0.1/g5_meeting_api/post.php                │
│         (게시글 1개 = 회의 요약)                                 │
│       → http://127.0.0.1/g5_meeting_api/comment.php             │
...
# 그누보드5 API
G5_API_BASE=http://127.0.0.1/g5_meeting_api
...
#### 로컬 (XAMPP)
이미 `c:\dev2\gnuboard5\plugin\meeting_api\` 에 설치되어 있습니다.
```powershell
curl http://127.0.0.1/gnuboard5/plugin/meeting_api/health.php
python doctor.py
```

#### 원격 (cafe24 등) — 4단계

**배포 패키지**: `c:\dev2\g5_meeting_api_dist\` (ZIP: `c:\dev2\g5_meeting_api_dist.zip`, 12KB)
...
3. **게시판 자동 생성 (1회)**:
   ```
   https://YOUR-DOMAIN/gnu5615/plugin/meeting_api/setup_board.php?token=YOUR_TOKEN
   ```
...
#### 기존 `c:\dev2\g5_meeting_api\` 정리 (선택)

이전 방식(그누보드5 옆 별도 폴더) 잔재. 선택사항:
```powershell
Remove-Item C:\xampp\htdocs\g5_meeting_api      # junction 정리
# Remove-Item C:\dev2\g5_meeting_api -Recurse   # 원본 폴더 (백업 후)
```
...
설치:
```powershell
pip install kiwipiepy        # 형태소 분석 활성화 (선택)
```
...
### 회의록 RAG 챗봇 (ask / qa_watcher)

키워드 검색을 넘어 **자연어로 질문하면 회의록 근거로 답변**합니다.
회의록을 청크로 잘라 Ollama 임베딩(`bge-m3`)으로 벡터화하고, 질문과 유사한
청크를 찾아(벡터 + FTS5 하이브리드) LLM이 출처를 표기하며 답변합니다.

**준비 (1회):**

```powershell
ollama pull bge-m3        # 임베딩 모델 (~1.2GB)
python ask.py --index     # 기존 회의록 전체 인덱싱 (신규 회의는 자동 인덱싱됨)
```
...
```powershell
python qa_watcher.py                # 폴링 시작 (기본 20초 주기)
python qa_watcher.py --once         # 한 번만 확인 (테스트용)
...
**해결**: API 헬스 체크로 확인
```powershell
curl http://127.0.0.1/g5_meeting_api/health.php
...
1. **API 토큰 강화**: `c:\dev2\g5_meeting_api\config.php`
   ```php
   define('MEETING_API_TOKEN', '강력한-32자-이상-랜덤-문자열');
...
Invoke-WebRequest http://127.0.0.1/g5_meeting_api/health.php
