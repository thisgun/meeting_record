...
> **⚠️ 안내:** 프로젝트명/디렉토리명/저장소명에 사용된 `metting`은 `meeting`의 오타입니다. 이미 그누보드5 게시판 `bo_table=metting`, PHP 폴더 `plugin/metting_api/`, 환경변수 `METTING_*` 등에 광범위하게 사용되고 있어 호환성 유지를 위해 그대로 둡니다. 새로 시작하시면 `meeting`으로 변경하셔도 좋습니다.
