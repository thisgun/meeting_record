# g5_metting_api — 그누보드5 회의록 등록 PHP 플러그인

회의록 자동 등록을 위한 그누보드5 plugin/ 표준 배치 패키지.

## 설치 위치

이 폴더의 `plugin/metting_api/` 를 그누보드5 의 `plugin/` 안에 통째로 업로드.
...
1. **FTP 업로드** — `plugin/metting_api/` 를 그누보드5 plugin 안에 그대로
2. **운영 토큰 설정** — `config.local.php.example` → `config.local.php` 복사 후 토큰 변경
3. **게시판 자동 생성** — `https://YOUR-DOMAIN/<그누보드폴더>/plugin/metting_api/setup_board.php?token=YOUR_TOKEN`
4. **setup_board.php 삭제** (보안)
5. **헬스체크** — `https://YOUR-DOMAIN/<그누보드폴더>/plugin/metting_api/health.php`
